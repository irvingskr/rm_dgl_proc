...
namespace RmDglProc{
    /// 实现绿色过滤
    void Processor::filterGreenColor(const unsigned char* image, int width, int height, int green_threshold) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                if (pixel[1] > pixel[0] + green_threshold && pixel[1] > pixel[2] + green_threshold) { // G > R and G > B
                    // 保留绿色像素，将其设置为白色
                    *pixel_filtered = 255;
                } else {
                    // 非绿色像素，将其设置为黑色
...
//        cv::Mat filteredImg(height_, width_, CV_8UC1, image_filtered_);
        ...
        for (int i = 0; i < circles_.size; ++i) {
            ...
            cv::line(filteredImg, cv::Point(circles_[i].center.x,circles_[i].center.y), cv::Point(circles_[i].center.x + circles_[i].radius,circles_[i].center.y), cv::Scalar(255), 1);
            std::cout<<"圆"<<i+1 <<" x:"<<circles_[i].center.x<<" y："<<circles_[i].center.y<<" 半径："<<circles_[i].radius<<std::endl;
        }
...
int main() {
#if TEST
    RmDglProc::Processor processor(1,1,GREEN_THRESHOLD,C_THRESHOLD);
    processor.loadImageData("/home/irving/图片/images.jpeg");
    processor.process();
